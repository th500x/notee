...
const { TACTICAL_GRID_WIDTH, TACTICAL_GRID_HEIGHT } = require('./tacticalBattleGrid.js');
const { createTerrainShapeRng, generateTerrainOccupancy, TERRAIN_OCC } = require('./terrainShape.cjs');
const { resolveWangBaseTileGrid } = require('./terrainWangResolve.cjs');
const {
  placeTerrainObjects,
  ...
  HILL_VARIANT_IDS,
  hillTileRel,
} = require('./terrainObjectPlace.cjs');
const { placeGameplayObjects } = require('./terrainGameplayObjects.cjs');
/**
 ...
 */
function generateMapV2(options = {}) {
 ...
 * @returns {object}
 */
function syncMapResultVisualsFromTerrain(mapResult, opts = {}) {
