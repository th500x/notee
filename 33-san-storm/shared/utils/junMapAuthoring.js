/**
 * 战略大地图作者层纯函数（31-1 · P0+）
 * 槽位校验、Meowa 尺寸、CDN URL 收集。须与 junMapAuthoring.cjs 同步。
 */

export const JUN_BATTLEFIELD_OBJECT = 'jun_battlefield';

export const LAYOUT_PROFILES = new Set(['2x1_v', '2x1_h', '2x2']);

/** slots.cities[].kind：与库 `cities.city_type` / 地图 object 四型对齐 */
export const SLOT_CITY_KINDS = new Set([
  'city_major',
  'city_medium',
  'city_small',
  'city_gate',
 ...
 * @returns {string[]}
 */
export function collectHttpUrls(value) {
  const out = new Set();
  function walk(v) {
    if (typeof v === 'string') {
 ...
 */
export function assertMeowaMapSize(map, expected) {
  const errors = [];
...
/**
 * @param {object|null|undefined} slots
 */
export function validateJunSlots(slots) {
  const errors = [];
  const warnings = [];
