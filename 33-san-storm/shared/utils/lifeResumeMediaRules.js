...
/** file input accept：MIME + 常见扩展名（部分浏览器对 Office 仅认扩展名） */
export const LIFE_DOCUMENT_ACCEPT =
...
/**
 * 文档 MIME：优先 file.type；空时按扩展名推断（Windows 选文件常见）
 */
export function resolveDocumentMimeType(mimeType, filename) {
...
export function validateMediaUploadRequest({ mediaType, mimeType, sizeBytes, skipSizeCheck = false, filename = null }) {
...
/**
 * @param {'none'|'photos'|'video'|'document'} bundleType
