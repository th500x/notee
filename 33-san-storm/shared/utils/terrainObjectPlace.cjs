...
const { TERRAIN_OCC } = require('./terrainWangTables.cjs');
/** 对象瓦相对 assets/san_1_map/ */
const TERRAIN_OBJECT_DIR = 'tile_2_terrain';

/** 单格山丘叠章上限（超过易糊） */
const HILL_STAMPS_PER_CELL_MAX = 3;

/** 河格数大于此值才尝试放桥（水格 4～6 → 一座） */
const BRIDGE_MIN_RIVER_CELLS = 3;

/** 河格数大于此值时尝试放第二座桥（水格 >6 → 两座） */
const BRIDGE_DUAL_MIN_RIVER_CELLS = 6;

/** 两座桥中点曼哈顿距离下限（避免贴在一起） */
const BRIDGE_PAIR_MIN_SEPARATION = 3;

/** 桥距地图外缘至少空出的格数（靠边无意义） */
const BRIDGE_EDGE_MARGIN = 1;

/** @typedef {{ id: string, spanW: number, spanH: number, tileRel: string }} ForestVariantDef */

const FOREST_VARIANT_DEFS = Object.freeze([
  Object.freeze({
    id: '01',
    spanW: 2,
    spanH: 1,
...
const HILL_VARIANT_IDS = Object.freeze(['01', '02']);

/** @typedef {'ew'|'ns'} BridgeOrient — ew=横桥(01) 跨南北河；ns=竖桥(02) 跨东西河 */

const BRIDGE_DEFS = Object.freeze({
  ew: Object.freeze({
    orient: 'ew',
    variant: '01',
 ...
 * @returns {number}
 */
function countWaterCells(occupancy) {
  let n = 0;
 ...
 * }}
 */
function placeRiverBridge(options) {
 ...
 * }}
 */
function placeTerrainObjects(options) {
  const occupancy = options?.occupancy;
 ...
 * @returns {string[][]}
 */
function buildGameplayTerrainFromOccupancy(occupancy, overlays) {
  const height = occupancy.length;
...
module.exports = {
  TERRAIN_OBJECT_DIR,
  HILL_STAMPS_PER_CELL_MAX,
  BRIDGE_MIN_RIVER_CELLS,
  BRIDGE_DUAL_MIN_RIVER_CELLS,
  BRIDGE_PAIR_MIN_SEPARATION,
  BRIDGE_EDGE_MARGIN,
  FOREST_VARIANT_DEFS,
  HILL_VARIANT_IDS,
  BRIDGE_DEFS,
