...
import LargeMapTile from './LargeMapTile';
import LargeMapUnitsOverlay from './LargeMapUnitsOverlay';
import { manualHighlightForTacticalCell } from '@/battle/manualHighlightModel';
import {
  buildTroopTooltipContent, buildMapCellTooltipInfo, sumBattleFactionTroopTotals,
...
import './LargeMapGrid.css';

/**
 * 叠加框必须用 absolute 叠在格子上方，不可作为 grid 子项（否则与格子争位，地图会挤到网格外）。
 *
 * Tooltip 架构（v2：与小型地图 BattleMap 统一）
 * ──────────────────────────────────────────────
 * 每格 LargeMapTile 的根节点同时承担引擎宿主（data-battle-y/x）和事件源（onMouseEnter）。
 * 引擎在运行期写入 data-troop；hover 时直接从 dataset.troop 读取，优先级：
 *   部队 → 地形/对象/特效
 * 不再使用 elementsFromPoint，杜绝叠层/z-index 导致的命中漂移。
 */
const LargeMapGrid = forwardRef(function LargeMapGrid(
  {
    cells,
    seed,
    title = '战场地图',
  ...
  const gridStyle =
    gridCols > 0 && gridRows > 0
      ? { '--lm-cols': gridCols, '--lm-rows': gridRows }
      : undefined;
  ...
  const handleHover = useCallback((e) => {
    ...
    const cell = cl[y]?.[x];
    const info = cell ? buildMapCellTooltipInfo(cell) : null;
  ...
  return (
    <div className="largemap-card">
      <div className="largemap-aligned-stack">
        {title && (
          <div className="largemap-title" style={{ position: 'relative' }} ref={mapTitleBlockRef}>
            <button
              type="button"
              className="map-title-label-btn"
              aria-expanded={mapInfoOpen}
              aria-controls="largemap-gen-info"
              title="点按查看地图信息"
            ...
            {manualActionHintText ? (
              <span className="largemap-action-hint">{manualActionHintText}</span>
            ) : null}
            {mapInfoOpen && meta ? (
              <div id="largemap-gen-info" className="map-label-info-pop" role="dialog" aria-label="地图信息">
                {meta}
              </div>
            ) : null}
          </div>
        )}
        <div className="largemap-meta map-meta-troops" aria-live="polite">
        ...
        >
          ...
          {manualChrome ? (
            <div
              className="largemap-manual-chrome"
        ...
        {/* 地形/部队浮动提示：挂 body，避免 largemap-wrap 滚动层裁切 */}
