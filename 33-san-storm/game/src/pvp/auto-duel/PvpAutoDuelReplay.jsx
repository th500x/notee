/**
 * PVP 自动对决 · 简化回放（原 SiegeReplayMini，17-5 §12.14 改名）
 * 左=攻方、右=守军；仅解析带 [攻方]/[守军] 的推演原文（与 pvpAutoDuelSim 文本日志同源）。
 * 仅用于历史无 `eventReplay` 的战报兜底；新战报走事件流壳 PvpTacticalBattleShell。
 */
 ...
 */
export function parsePvpAutoDuelReplayTimeline(battleLog) {
  const raw =
...
export default function PvpAutoDuelReplay({
  open,
  onClose,
  battleLog,
  leftLabel = '攻方',
  ...
  const { timeline, animSteps } = useMemo(
    () => parsePvpAutoDuelReplayTimeline(battleLog),
    [battleLog],
  );
