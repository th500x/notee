...
生成方式：扫描 `backend/database/migrations/*.sql` 中含 `created_at` 或 `updated_at` 的表定义，并与 `33-san-storm/backend/**/*.js` 中的列名引用交叉比对（未扫前端 `game/`：当前无直接引用）。  
...
3. [ ] 同步更新 `33-san-storm/docs/00/00-base/01-database-split/` 对应分册表结构。  
...
```bash
rg "created_at|updated_at" 33-san-storm/backend/database/migrations --glob "*.sql"
rg "created_at|updated_at" 33-san-storm/backend --glob "*.js"
```
