/**
 * Apply 003-add-tax-sheet.sql using backend/.env
 *
 * Run: node backend/scripts/apply-tax-sheet-migration.js
 *   or cd backend && npm run migrate:tax
 */
...
async function main() {
  const sqlPath = path.join(backendRoot, 'database/migrations/003-add-tax-sheet.sql');
  ...
  try {
    ...
    console.log('OK: tax_sheet column applied.');
  } catch (err) {
    if (err.code === 'ER_DUP_FIELDNAME' || err.errno === 1060) {
      console.log('SKIP: tax_sheet already exists.');
      process.exit(0);
    }
