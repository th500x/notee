...
export default function TaxSheetPage({ project, onBack, onSaved, onProjectSynced }) {
  ...
  const closeExportPicker = useCallback(() => {
  ...
  return (
    <div className="w-full min-w-0 space-y-6">
      ...
      <div className="grid w-full min-w-0 gap-3 grid-cols-3">
        ...
        >
          <span>📊</span>
          <span>{exporting ? 'Exporting…' : '导出项目'}</span>
        </button>
      </div>
      ...
      {exportPickerOpen ? (
        ...
        >
          <div
            ...
            role="dialog"
            aria-labelledby="tax-export-picker-title"
          >
            <div className="flex items-center justify-between p-6 border-b">
              <h3 id="tax-export-picker-title" className="text-xl font-semibold text-gray-900">
                Rows to export
              </h3>
              <button
                type="button"
            ...
            <ul className="px-6 pb-4 max-h-[min(50vh,320px)] overflow-y-auto space-y-2 border-b border-gray-100">
              {sheet.rows.map((row, idx) => {
                const label =
                  (row.room && String(row.room).trim()) || `Row ${idx + 1} (no ROOM)`
                return (
                  <li
                    key={row.id}
