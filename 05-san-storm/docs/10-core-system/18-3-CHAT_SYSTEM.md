...
**同步说明**：按项目规范 **`docs/` 目录不提交 GitHub**（见仓库根 `.gitignore` 与 [DEVELOPMENT_RULES.md](../DEVELOPMENT_RULES.md) §Git 同步）；本文档仅在本地维护。聊天实装以仓库内 `05-san-storm/backend/`、`game/` 代码为准。生产部署需执行 `create-chats-table.sql` 等迁移（见 §「实装对照」）。
...
**拉取列表**：按频道查询最近 **100** 条（SQL `ORDER BY created_at DESC`，**最新一条在结果数组首位**）。前端 **天下 / 势力 / 军团** 三频道列表自上而下展示时，**最新消息在最上方**，更旧消息在下；输入框仍在底部。**当前阶段**：直接读 **MySQL**；不依赖 Redis。
...
- **文档**：实装对照改为全栈说明；生产须跑迁移、轮询与后续 WebSocket/Redis 边界（`docs/` 不纳入远程仓库）。
