...
RESOLUTION_OPTIONS = {
    ...
    "6": (64, 128, "64x128 - 功能图片"),
    "7": (256, 128, "256x128 - 功能图片"),
    "8": (36, 36, "36x36 - 功能图片"),
    "9": (48, 48, "48x48 - 功能图片"),
    "0": (128, 128, "128x128 - 功能图片"),
}
...
def main():
    ...
    # 获取当前目录及子目录下的所有PNG文件
    png_files = sorted(current_path.rglob("*.png"))
    
    # 排除脚本自身
    png_files = [f for f in png_files if f.name not in ['resize-png.py']]
    
    if not png_files:
        print("当前目录及子目录未找到PNG文件！")
        input("\n按回车键退出...")
        return
    
    print(f"找到 {len(png_files)} 个PNG文件（含子目录）")
    print()
    ...
    for i, file_path in enumerate(png_files, 1):
        ...
        if parent != current_dir:
            current_dir = parent
            dir_label = "." if parent == "." else parent
            print(f"\n  [{dir_label}]")
    ...
    while True:
        choice = input("请输入选项 (1/2/3/4/5/6/7/8/9/0): ").strip()
