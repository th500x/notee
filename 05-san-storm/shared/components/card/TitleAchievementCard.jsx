 ...
 * 称号/成就卡牌组件（共享版本）
 *
 * 尺寸: 256 × 192 px（与装备件卡牌一致）
 * 背景: 复用 bg_r1~bg_r5.png，backgroundPosition: center 截取中间段
 *
 * 布局（仿 EquipmentCard）:
 *   顶部名称区   40px  — 类型图标 + 名称 + 稀有度标签
 *   属性加成区   72px  — attributeBonus 列表（中文标签 + 数值）
 *   特效/描述区  80px  — 特殊效果（中文）或描述
 */
const TitleAchievementCard = ({
  item,
  type = 'title',
  baseUrl = '',
  onSelect,
}) => {
  // ── 稀有度配色（与 EquipmentCard 完全一致）──────────────────
  const rarityColors = {
  ...
  const typeInfo = typeMap[type]              || typeMap.title;

  // ── 背景图路径 ───────────────────────────────────────────────────
  const rarityToFilename = {
    common: 'bg_r1', rare: 'bg_r2', epic: 'bg_r3', legendary: 'bg_r4', core: 'bg_r5',
  };
  const bgUrl = `${baseUrl}assets/san_1_ui_card/bg/${rarityToFilename[item.rarity] || 'bg_r1'}.png`;

  // ── attributeBonus: 对象 {key: value} → 数组 [{key, value}] ──────
  const bonusList = item.attributeBonus
    ? Object.entries(item.attributeBonus).map(([key, value]) => ({ key, value }))
    : [];
                ...
                return (
                  <div key={i} className="flex items-center gap-1 text-xs">
                    ...
                    <span className="text-gray-900 font-bold">
                      {val > 0 ? `+${(val / 10).toFixed(1)}` : (val / 10).toFixed(1)}
                    </span>
                  </div>
                );
              })}
...
export default TitleAchievementCard;
