 ...
 * 前端 ESM 入口；算法须与 `cityInitialAttributes.cjs` 同步。
 */

/** @typedef {'city_small'|'city_medium'|'city_major'|'city_gate'} CityType */

/** 各类型 [下限, 上限]；关隘商农军文钉死 0 */
export const CITY_ATTR_BOUNDS = Object.freeze({
...
export const GATE_FIXED_ZERO_DIMS = Object.freeze(['trading', 'farming', 'military', 'culture']);

/** 开服：下限 × 100%～105%（含两端） */
export const INIT_FLOOR_RATIO_MIN = 1;
export const INIT_FLOOR_RATIO_MAX = 1.05;

/** 日成长掷骰：+1% / +2% / +3% */
export const DAILY_GROWTH_ROLLS = Object.freeze([
...
export const NPC_GARRISON_POPULATION_RATIO = 0.01;

/** 编制硬顶（与 `buildNpcUnitsForCityRow` override 一致） */
export const NPC_GARRISON_CAP_HARD_MAX = 2000;

/** 每日 0:00：损兵城按编制上限恢复本比例（四舍五入；PVE/PVP 统一） */
export const NPC_DAILY_RECOVERY_RATIO = 0.1;
 ...
 * @returns {number}
 */
export function resolveNpcGarrisonCapFromPopulation(population) {
 ...
 * @returns {number}
 */
export function resolveNpcDailyRecoveryAdd(cap) {
...
/**
 * @param {unknown} cityType
 * @returns {CityType|null}
 */
export function normalizeCityType(cityType) {
...
/**
 * @param {unknown} cityType
 * @returns {typeof CITY_ATTR_BOUNDS[CityType]|null}
 */
export function getCityAttrBounds(cityType) {
...
/**
 * 下限 × [100%, 105%] 取整（含两端）。
 * @param {number} floorLo
 * @param {() => number} [rng]
 */
export function rollInitialFromFloor(floorLo, rng = Math.random) {
 ...
 * @returns {1|2|3}
 */
export function rollDailyGrowthPercent(rng = Math.random) {
 ...
 * @param {number} hi
 * @returns {{ value: number, grew: boolean }}
 */
export function applyPercentGrowth(current, percent, hi) {
 ...
 * 开服 / 换季初值（关隘四维恒 0）。
 * @param {unknown} cityType
 * @param {{ specialResourceTrading?: number, specialResourceFarming?: number, rng?: () => number }} [opts]
 */
export function buildInitialCityAttributes(cityType, opts = {}) {
...
/**
 * 已占城一日成长（各字段独立掷骰；已达上限跳过）。
 * @param {{ city_type?: string, cityType?: string, population?: number, trading?: number, farming?: number, military?: number, culture?: number, defense?: number, special_resource_trading?: number, specialResourceTrading?: number, special_resource_farming?: number, specialResourceFarming?: number }} row
 * @param {{ rng?: () => number }} [opts]
 */
export function growOwnedCityAttributes(row, opts = {}) {
