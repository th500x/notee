 ...
 * 须与 terrainGameplayObjects.js 同步。
 *
...
const GAMEPLAY_OBJECT_DIR = 'tile_3_object';

const GAMEPLAY_OBJECT_DEFS = Object.freeze({
  rock: Object.freeze({
    isPassable: false,
...
const CHEST_LOOT_BY_VARIANT = Object.freeze({
  '01': Object.freeze(['common', 'rare']),
  '02': Object.freeze(['epic', 'legendary']),
});

function chestTileRel(variant, isOpen = false) {
 ...
 * @returns {'01'|'02'}
 */
function pickChestVariantForBattleRarity(battleRarity) {
 ...
 */
function buildCenterForbiddenKeys(width, height, ratio = 0.3) {
  const cx = (width - 1) / 2;
  const cy = (height - 1) / 2;
 ...
 * }}
 */
function placeGameplayObjects(options = {}) {
...
module.exports = {
  GAMEPLAY_OBJECT_DIR,
  GAMEPLAY_OBJECT_DEFS,
  CHEST_LOOT_BY_VARIANT,
  chestTileRel,
  pickChestVariantForBattleRarity,
  buildCenterForbiddenKeys,
