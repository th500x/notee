...
const EXPLORE_PUNISH_BATTLE_LOCK_KIND = 'punish_battle';

/**
 * @param {unknown} sessionLock
 * @returns {null | { kind: string, eventId: string, optionKey: 'A'|'B', lockedFortune: object }}
 */
function parseExplorePunishBattleLock(sessionLock) {
 ...
 */
function buildExplorePunishBattleLock({ eventId, optionKey, lockedFortune }) {
 ...
 */
function isPendingPunishRewardRequest(lock, eventId, optionKey, battleResult) {
...
function lockedFortuneToInternal(lockedFortune) {
  return {
...
function fortuneToApiPayload(fortune) {
  return {
...
module.exports = {
  EXPLORE_PUNISH_BATTLE_LOCK_KIND,
