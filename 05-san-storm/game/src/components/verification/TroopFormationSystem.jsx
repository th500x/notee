...
const TroopFormationSystem = () => {
  ...
  const { troops, loading: troopsLoading, error: troopsError } = useTroops();
  ...
  if (charactersError || troopsError) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-lg p-6">
        <p className="text-red-600">❌ 加载失败: {charactersError || troopsError}</p>
      </div>
    );
  }
