  ...
  try {
    ...
    if (e && (e.code === 'ER_BAD_FIELD_ERROR' || /Unknown column/i.test(e.message || ''))) {
      return {
        ...
        status: 503,
        error:
          '数据库缺少主城列（main_city_id / main_city_changed_at）。请在 MySQL 执行 05-san-storm/backend/database/migrations 下的 add-players-main-city-id.sql 与 add-players-main-city-changed-at.sql，或在 backend 目录运行：node scripts/apply-pending-local-ddl.js',
      };
    }
