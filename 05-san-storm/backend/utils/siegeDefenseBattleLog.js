 ...
 * 解析对象：siegePvpSkirmish 产出的 battleLog 行（须含 [攻方]/[守军] 标签）
 */
 ...
 */
function formatSkirmishLineForDefender(line, tacticalRound = null) {
  ...
  const body = (battleLogLines || [])
    .map((line) => {
      ...
      if (m) {
        ...
        return formatSkirmishLineForDefender(line, tacticalRound);
      }
      return formatSkirmishLineForDefender(line, tacticalRound);
    })
    .join('\n');
