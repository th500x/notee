...
const battleSchemas = require('../middleware/validationSchemas/battles');
 ...
 */
router.get('/', validateQuery(battleSchemas.listQuery), async (req, res, next) => {
  try {
 ...
 */
router.post('/favorite', validateBody(battleSchemas.favoriteBody), async (req, res, next) => {
  try {
 ...
 * Body: { playerId, battleId }
 */
router.post('/unfavorite', validateBody(battleSchemas.favoriteBody), async (req, res, next) => {
  try {
    const { playerId, battleId } = req.body;
 ...
 * GET /api/battles/:id
 */
router.get('/:id', validateParams(battleSchemas.battleIdParam), async (req, res, next) => {
  try {
 ...
 * POST /api/battles
 */
router.post('/', validateBody(battleSchemas.saveBattleBody), async (req, res, next) => {
  try {
    ...
    // 章节：客户端上报自动战斗银两 + 出征粮草（避免与事件奖励/攻城结算重复计数）
    if (battleType === 'pve_chapter') {
