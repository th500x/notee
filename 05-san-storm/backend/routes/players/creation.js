...
router.post('/generate-attributes', withRoute('生成属性方案失败', async (req, res) => {
  const { rarity = 'common' } = req.body;
  const options = await PlayerService.generateAttributeOptions(rarity);
  res.json({ success: true, data: { options } });
}));

router.post('/validate-name', withRoute('验证角色名失败', async (req, res) => {
...
router.post('/create', withRoute('创建角色失败', async (req, res) => {
  const {
...
router.get('/:playerId/troops/initial', withRoute('获取初始部队选项失败', async (req, res) => {
  const { factionId } = req.query;
  if (!factionId) return res.status(400).json({ success: false, error: '缺少势力ID' });
...
router.post('/:playerId/creation-progress', withRoute('保存角色创建进度失败', async (req, res) => {
  await playerCreationService.saveCreationProgress(req.params.playerId, req.body);
  res.json({ success: true, message: '进度已保存' });
}));

router.post('/:playerId/generate-attributes-batch', withRoute('生成属性批次失败', async (req, res) => {
  const { rarity = 'common' } = req.body;
...
router.post('/:playerId/select-option', withRoute('选择属性方案失败', async (req, res) => {
