...
assert.strictEqual(DURATION_MIN_SEC, 1800);
...
assert.strictEqual(
  assertPourPayload({ ...ok, kinds: ['beer', 'whisky', 'wine'] }).kinds.length,
  3
);
...
throwsCode(
  () => assertPourPayload({ ...ok, people: 2, names: ['CHRIS', 'SOLO'] }),
  'BAD_POUR'
);
assert.strictEqual(
  assertPourPayload({ ...ok, people: 1, names: ['SOLO'] }).names[0],
  'SOLO'
);
