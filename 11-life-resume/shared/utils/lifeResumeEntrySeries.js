 ...
 * 须与 lifeResumeEntrySeries.cjs 同步
 */

export const CHRONOLOGICAL_ENTRY_SERIES_ID = null;
export const CHRONOLOGICAL_ENTRY_SERIES_KEY = 'chronological';
export const CHRONOLOGICAL_ENTRY_SERIES_NAME = '编年历';
export const MAX_CUSTOM_ENTRY_SERIES_PER_USER = 2;
export const ENTRY_SERIES_NAME_MAX_CJK = 5;

/**
 * @param {unknown} raw
 * @returns {number|null}
 */
export function normalizeEntrySeriesId(raw) {
...
/**
 * @param {number|null|undefined} entrySeriesId
 * @returns {string}
 */
export function entrySeriesStorageKey(entrySeriesId) {
...
/**
 * @param {string} name
 * @returns {{ ok: true, name: string } | { ok: false, error: string, code: string }}
 */
export function validateEntrySeriesName(name) {
...
/**
 * @param {Array<{ entrySeriesId?: number|null }>} entries
 * @param {number|null} entrySeriesId
 */
export function filterEntriesByEntrySeriesId(entries, entrySeriesId) {
...
/**
 * @param {Array<{ id: number, name: string, sortOrder?: number }>} customSeries
 */
export function buildEntrySeriesSwitcherList(customSeries = []) {
...
/**
 * 访客可见系列：编年历 + 至少有一条可见条目的自定义系列。
 * @param {Array<{ entrySeriesId?: number|null }>} visibleEntries
 * @param {Array<{ id: number, name: string, sortOrder?: number }>} customSeries
 * @param {boolean} isOwner
 */
export function buildVisibleEntrySeriesList(visibleEntries, customSeries, isOwner) {
  if (isOwner) {
