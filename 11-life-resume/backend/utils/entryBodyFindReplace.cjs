/**
 * 片段正文（body）搜索与替换算法 — Node 后端 CommonJS
 * 须与 src/utils/entryBodyFindReplace.js 保持同算法
 *
 * 搜索词上限：中文汉字与非汉字字符分别累计，任一侧超限即不允许搜索。
 * 匹配不区分大小写；替换文字按用户输入原样写入。
 */

const FIND_QUERY_MAX_CHINESE = 5;
const FIND_QUERY_MAX_OTHER = 10;

const HAN_CHAR = /\p{Script=Han}/u;

function analyzeFindQuery(find) {
...
function matchesFindQuery(body, find) {
  return countMatches(body, find) > 0;
}

function applyBodyReplace(body, find, replace) {
  const text = String(body ?? '');
